...
export class Settings {
  ...
  public getSettings(
    ...
    if (result.size === SizeType.STRETCH) {
      if (result.minWidth === undefined || result.minWidth <= 0)
        result.minWidth = 100;

      if (result.maxWidth === undefined || result.maxWidth < result.minWidth)
        result.maxWidth = 2000;

      if (result.minHeight === undefined || result.minHeight <= 0)
        result.minHeight = 100;

      if (result.maxHeight === undefined || result.maxHeight < result.minHeight)
