...
const Page = React.forwardRef<HTMLDivElement, Props>((props, ref) => {
  return (
    <div className={`page page-${props.number} ${props.className ? props.className : ''}`} style={{ color: 'red', backgroundColor: 'blue' }} ref={ref}>
      ...
      {props.children}
      {props.image ? <div className='page-footer' style={{ position: 'fixed', bottom: 0, fontSize: 'calc(2vh)', margin: '10px', color: 'white', textShadow: '1px 1px 2px black' }}>{props.number} / <span className='page-total'>-</span></div> : ''}
    </div>
  );
});
